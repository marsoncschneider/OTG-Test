...
function Player.feed(self, food)
	local foodCondition = self:getCondition(CONDITION_REGENERATION, CONDITIONID_DEFAULT)
	...
	local hpticks = vocation:getHealthGainTicks()
	local manaticks = vocation:getManaGainTicks()
	local check = getPlayerStorageValue(self, 25896) - os.time()
	local mult = 1
		
	--hpticks = hpticks / 2 --neste exemplo os ticks cairam pela metade, acelerando o regen
	...
	if foodCondition then
		
		local soma = foodCondition:getTicks() + (food * 1000)
		
		self:removeCondition(CONDITION_REGENERATION, CONDITIONID_DEFAULT)
		foodCondition = Condition(CONDITION_REGENERATION, CONDITIONID_DEFAULT)
				
		foodCondition:setTicks(soma)
		foodCondition:setParameter(CONDITION_PARAM_HEALTHGAIN, healthgain * mult)
		foodCondition:setParameter(CONDITION_PARAM_HEALTHTICKS, hpticks * 1000)
		foodCondition:setParameter(CONDITION_PARAM_MANAGAIN, managain * mult)
		foodCondition:setParameter(CONDITION_PARAM_MANATICKS, manaticks * 1000)
