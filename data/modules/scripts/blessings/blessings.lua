...
Blessings.sendBlessDialog = function(player)
	...
	for k = 1, #Blessings.All do
		...
		if v.type ~= Blessings.Types.PvP or Blessings.Config.HasToF then
			...
			-- Aqui
			if (player:getStorageValue(30056) == -1) then
				player:setStorageValue(30056, 0)
			end
			if (player:getStorageValue(30057) == -1) then
				player:setStorageValue(30057, 0)
			end
			-- END OF AQUI
			local sum = player:getStorageValue(30056) --FreeBless
			local sum2 = player:getStorageValue(30057) --PremiumBless
			
			if player:getBlessingCount(v.id) >= 1 then
			msg:addByte(sum+sum2) --total de bless que ja foram compradas
			--msg:addByte(player:getBlessingCount(v.id)) -- Store Blessings Count ACTIVE
			msg:addByte(player:getBlessingCount(v.id)) -- Bless Compradas pela store
			else
			msg:addByte(0)
			msg:addByte(0)
			end
...
Blessings.BuyAllBlesses = function(player)
	...
	if player:removeMoneyNpc(totalCost) then
		...
		--Aqui adicionar na storage da blessFree
		
		if player:getStorageValue(30056) == -1 then
			player:setStorageValue(30056, 0)
		end
		local storeBlessCount = player:getStorageValue(30056)
		player:setStorageValue(30056, storeBlessCount + 1)
		--END Aqui adicionar na storage da blessFree
		local version = player:getClient().version
