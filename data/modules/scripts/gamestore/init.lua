...
function Player.canBuyOffer(self, offer)
	...
	if disabled ~= 1 then
		...
		elseif offer.type == GameStore.OfferTypes.OFFER_TYPE_ALLBLESSINGS then
			local blessQtd = 0
			for i = 1, 8 do
				if self:getBlessingCount(i) >= 1 then
					blessQtd = blessQtd + 1
				end
			end
			if blessQtd >= 5 then
...
function GameStore.processAllBlessingsPurchase(player, count)
	...
	--aqui contar as bless compradas pela store
	if player:getStorageValue(30057) == -1 then
		player:setStorageValue(30057, 0)
	end
	local storeBlessCount = player:getStorageValue(30057)
	player:setStorageValue(30057, storeBlessCount + 1)
	--end of aqui contar as bless compradas pela store
	local version = player:getClient().version
