...
local function sumonarFieryHearts()
	...
	for _, creature in pairs(spectator) do
		if creature:isMonster() then
			...
			if creature:getName():lower() == "the duke of the depths immortal" then
				boss = true
				bossId = Creature(creature:getId())
				auximmortal = 1
			end
	...
	if boss == true and contagemHearts == 0 then
		...
		bossId:remove()
		local newBoss = false
		if auximmortal == 0 then
		newBoss = Game.createMonster("the duke of the depths immortal", oldBossPosition, false, true)
	
		else
		newBoss = Game.createMonster("the duke of the depths", oldBossPosition, false, true)

		end
		if newBoss then
			newBoss:registerEvent("fieryHearts")
			newBoss:registerEvent("theDukeHealFireDamage")
			--newBoss:addHealth(-(newBoss:getHealth() - oldBossHealth))
	
		newBoss:setHealth(oldBossHealth)
...
function dangerousDepthCrystals.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	if not Item(item.uid) then
	return false
	end

	if Creature(target.uid) then
	return false
	end
