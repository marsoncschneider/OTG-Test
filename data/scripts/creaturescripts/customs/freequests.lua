...
--Marson Carlos Schneider 19/08/21 - 21:43
--TODO:
--1 Utilizar lua tables para fazer a conversão da string "storageId"
--2 Por questões de otimização, apenas setar os valores no banco, em caso de novas quests, aumentar o "upperLimit" será suficiente
--3 fazer a chamada de uma função/loop (MLoadBalancerStg), onde a mesma trabalhará com numero #questTable, iniciando em 1 ou (numero atual da sequencia do processo), e finalizando no valor total da tabela, tempo inicial poderá ser de 1000ms
local cpuload = 10

function freeQuests.onLogin(player)
	i = 1
	if configManager.getBoolean(configKeys.FREE_QUESTS) and player:getStorageValue(Storage.FreeQuests) < upperLimit  or true then -- esse or true faz o script rodar com qualquer player, SO PARA TESTE
		if #stgWorkerA < 1 then
		for index, value in pairs(questTable) do
			if value.storageId and value.storageValue then
				--print(value.storageId, value.storageValue, i) --print que mostra a leitura das tabelas
				table.insert(stgWorkerA, value.storageId)
				table.insert(stgWorkerB, value.storageValue)
				--print('provareal', stgWorkerA[i], stgWorkerB[i]) --print que mostra como gravaram as tabelas
				i = i + 1
			end
		end
		end
		MLoadBalancerStg(player:getGuid(), 1) --mandar o valor inicial para o processo iniciar
	end
	return true
end
freeQuests:register()

function MLoadBalancerStg(temp, row)
	--print('MLoadBalancerStg says: Hello world.. row = '..row..'') --print que mostra o estado atual/inicial da etapa fila de gravação
	for i = row, row+cpuload do
		local player = Player(temp)
		if not player then
		return true
		end
		--print('dbg', i, stgWorkerA[i], stgWorkerB[i]) --print que mostra a gravação atual
		if stgWorkerA[i] and stgWorkerB[i] then
			--if player:getStorageValue(stgWorkerA[i]) < stgWorkerB[i] then
				--player:setStorageValue(stgWorkerA[i], stgWorkerB[i])
				db.query("INSERT IGNORE INTO `player_storage` (`player_id`, `key`, `value`) VALUES ('"..temp.."', '"..stgWorkerA[i].."', '"..stgWorkerB[i].."')") --novo metodo
			--end
		end
	end
	if row+cpuload < #questTable then
		addEvent(MLoadBalancerStg, 1000, temp, row+cpuload) --1000 = 1 segundo
	else
		temp:setStorageValue(Storage.FreeQuests, upperLimit)
	end
end