...
local function checkHouseAccess(hireling)
	...
	--jose--lamp:setSpecialAttribute(HIRELING_ATTRIBUTE, hireling:getId()) --save hirelingId on item
	player:save()
	hireling.active = 0
	hireling.cid = -1
...
function Hireling:returnToLamp(player_id)
	...
	--jose--lamp:setSpecialAttribute(HIRELING_ATTRIBUTE, self:getId()) --save hirelingId on item
...
function PersistHireling(hireling)
	...
	--local query = string.format("SELECT `id` FROM `player_hirelings` WHERE `player_id`= %d", hireling.player_id)
...
function Player:findHirelingLamp(hirelingId)
	...
	for i=0,lastIndex do
		...
		if item and item:getId() == HIRELING_LAMP_ID and id == hirelingId then
			return item
		end
	end
	return nil
