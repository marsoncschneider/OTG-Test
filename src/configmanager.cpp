...
bool ConfigManager::load()
{
	...
	if (!loaded) { //info that must be loaded one time (unless we reset the modules involved)
		...
		string[MAP_NAME] = getGlobalString(L, "mapName", "otg-br");
		string[MAP_AUTHOR] = getGlobalString(L, "mapAuthor", "OTG-BR");
		...
		string[MAP_CUSTOM_AUTHOR] = getGlobalString(L, "mapCustomAuthor", "OTG-BR");
	...
	integer[SPOOF_GAIN] = getGlobalNumber(L, "spoofGain", 1);
